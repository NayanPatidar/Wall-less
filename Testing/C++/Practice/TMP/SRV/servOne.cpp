...
void handleX11Events(Display *display, int clientSocket) {
...
int main(){
    ...
    while (true)
    {
        ...
        const char* coorToSend = coordinates.c_str();
        int dataLength = coordinates.length();
    ...
    // std::thread x11Thread(handleX11Events, display, clientSocket);
    // x11Thread.join();
